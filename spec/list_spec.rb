...
describe(List) do
  ...
  describe("#tasks") do
    it("returns an array of tasks for that list") do
      test_list = List.new({:name => "Epicodus stuff", :id => nil})
      test_list.save()
      test_task = Task.new({:description => "learn SQL", :list_id => test_list.id(), :due_date => "1/21/2015"})
      test_task.save()
      test_task2 = Task.new({:description => "Review Ruby", :list_id => test_list.id(), :due_date => "1/21/2015"})
      test_task2.save()
  ...
  describe('#==') do
    it('is the same list if it has the same name') do
      ...
      expect(list1).to(eq(list2))
        ...
        # why not set up id on to do specs?
end
